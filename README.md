...
2. Запустите приложение. macOS покажет окно «Файл “PlannerApp” не был открыт» — нажмите
   **«Готово»** (не «Переместить в Корзину»).
...
Если кнопки «Всё равно открыть» в настройках нет (например, приложение переустановили поверх
старого), отметку «скачано из интернета» можно снять вручную — приложение останется на месте:

```bash
xattr -dr com.apple.quarantine /Applications/PlannerApp.app
```
